...
const Search = () => {
  ...
  const handleAdvancedSubmit = async (e) => {
    ...
    try {
      ...
      setHasMore(
        data.items && data.items.length === 10 && data.total_count > 10,
      );
    } catch (err) {
      setError(true);
    } finally {
    ...
    try {
      ...
      setHasMore(
        data.items &&
          ...
          searchResults.length + data.items.length < data.total_count,
      );
    } catch (err) {
      setError(true);
    } finally {
          ...
          username && (
            <div className="bg-white rounded-lg shadow-lg p-6 text-center">
              <p className="text-gray-600 text-lg">
                No users found matching your criteria. Try different search
                terms.
              </p>
            </div>
          )}
